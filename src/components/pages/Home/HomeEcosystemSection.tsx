...
export default function HomeEcosystemSection() {
  ...
  const ecosystemBg = useMemo(() => {
    const section = homePage?.sectionImages.find(
      (item) => item.sectionKey === "ecosystem",
    );
    return section?.image
      ? getCourseImage(section.image)
      : FALLBACK_ECOSYSTEM_BG;
  }, [homePage?.sectionImages]);

  const ecosystemBgAlt = useMemo(() => {
    const section = homePage?.sectionImages.find(
      (item) => item.sectionKey === "ecosystem",
    );
    return section?.title ?? "Ecosystem background";
  }, [homePage?.sectionImages]);

  return (    <section className="py-32 bg-brand-navy relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-20">
        <div className="text-center max-w-3xl mx-auto mb-20">
          ...
          <h3 className="text-4xl lg:text-5xl font-display font-bold text-white mb-6">Our Ecosystem</h3>
        ...
        <div className="grid lg:grid-cols-3 gap-8 items-stretch relative">
          {ECOSYSTEM_CARDS.map(({ icon: Icon, title, description, tag, variant }, idx) => (
            ...
            >
              ...
              >
                <Icon className={`w-6 h-6 ${variant === "highlight" ? "text-brand-navy" : "text-brand-cyan"}`} />
              </div>
              <h4 className="text-2xl font-display font-bold text-white mb-4">{title}</h4>
              <p className={`leading-relaxed mb-8 flex-1 relative z-10 ${variant === "highlight" ? "text-slate-300" : "text-slate-400"}`}>
                {description}
              </p>
              <div className={`pt-6 mt-auto ${variant === "highlight" ? "border-t border-brand-cyan/20" : "border-t border-white/10"}`}>
                <span className="text-xs font-bold text-brand-cyan uppercase tracking-widest">{tag}</span>
              </div>
            </motion.div>
          ))}
        </div>
        ...
        <div className="text-center mt-16">
          ...
          >
            Discover Our Ecosystem <ArrowRight className="w-5 h-5" />
          </Link>
        </div>
      </div>
    </section>
